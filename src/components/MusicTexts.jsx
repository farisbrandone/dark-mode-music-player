...
function MusicTexts() {
  ...
  return (
    <div
      className="w-full h-[98px] sm:h-[198px] p-4 flex flex-col items-center gap-4 overflow-hidden snap-center cursor-pointer"
    ...
    >
      {database.data[0]["music-text"].split(".").map((text, index) => (
        <p
          ...
          className={`${
            textStatus === text
              ? "text-[15px] text-[#E34715] sm:text-[17px]"
              : "text-[13px] sm:text-[15px] text-[#666666]//0.301"
          } text-[13px] sm:text-[15px]leading-[16px] font-[500] `}
          onClick={(text) => {
            setTextStatus(text);
          }}
        >
